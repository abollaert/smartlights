...
public final class DPWSClientImpl extends Client implements DPWSClient {
	...
	private static final Logger logger = Logger.getLogger(DPWSClientImpl.class
			.getName());
	...
	public DPWSClientImpl() {
		logger
				.info("Starting DPWS discovery process to discover light systems on the network...");
		...
		searchServiceParameter.addDeviceType(new QualifiedName(DEVICE_NAME,
				DEVICE_NAMESPACE));
		searchServiceParameter
				.addServiceType(new QualifiedName(
		...
		searchServiceParameter.addDeviceType(new QualifiedName(DEVICE_NAME,
				DEVICE_NAMESPACE));
		searchServiceParameter
				.addServiceType(new QualifiedName("LightControlService",
	...
	private final void loadBoards() throws DPWSException {
		...
		final AbstractAction getConfigurationAction = this.configurationService
				.getAction(GET_CONFIGURATION_ACTION_NAME,
						"ConfigurationService");
		...
		final Parameter response = getConfigurationAction
		...
		final PVI[] pathToNumberOfChannels = PVI
				.createPath("Board/NumberOfChannels");
		pathToNumberOfChannels[0].setIndex(0);
		pathToNumberOfChannels[1].setIndex(0);

		final PVI[] pathToChannelNumber = PVI
				.createPath("Board/Channel/ChannelNumber");
		...
		for (int i = 0; i < numberOfBoards; i++) {
			...
			final int boardId = Integer.valueOf(response
					.getValue(pathToBoardID).toString());
			final String boardDriverName = response.getValue(
					pathToBoardDriverName).toString();
			final String boardType = response.getValue(pathToBoardType)
					.toString();
			...
			if (boardType.equals("DIGITAL")) {
				...
				for (int j = 0; j < numberOfChannels; j++) {
					...
					final int channelNumber = Integer.valueOf(response
							.getValue(pathToChannelNumber).toString());
					final String lightName = response.getValue(pathToLightName)
							.toString();
	...
	public final void onServiceFound(final ISearchResult result) {
		logger.info("Service found ["
				+ result.getRemoteService().getServiceId() + "]");
